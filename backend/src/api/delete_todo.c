...
PRIVATE void __Constructor__(register void *const _self,
  ...
  register struct DeleteTodoApiHandler *const self = _self;
  if (ImParams_Match(args, 1u, PARAM_PTR) == IM_FALSE) {
    impanic("DeleteTodoApiHandler Constructor takes (struct FordoDB *)");
  }
...
__Handle__(register void *const _self,
  ...
  if (strcmp(HttpRequest_GetPath(request), "/api/delete-todo") != 0) {
    ...
    if (todo_id >= 0) {
      ...
      if (ImResInt_IsErr(owner_result) != IM_FALSE) {
        ...
        if (owner_uid != user_id) {
          ...
          register struct ImResVoid const result = FordoDB_DeleteTodo(self->db, todo_id);
...
PRIVATE void __InterfaceImplementation__(register void *const interface) {
  if (imisof(interface, HttpHandler) != IM_FALSE) {
    ...
    impanic("DeleteTodoApiHandler does not implement %s\n", imtype(interface));
  }
}

CLASS(DeleteTodoApiHandler) {
  _DeleteTodoApiHandler.size = sizeof(struct DeleteTodoApiHandler);
  _DeleteTodoApiHandler.ctor = __Constructor__;
  _DeleteTodoApiHandler.dtor = __Destructor__;
  _DeleteTodoApiHandler.implof = __InterfaceImplementation__;
}
