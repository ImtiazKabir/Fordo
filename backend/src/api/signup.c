...
PRIVATE void __Constructor__(register void *const _self,
  ...
  register struct SignupApiHandler *const self = _self;
  if (ImParams_Match(args, 1u, PARAM_PTR) == IM_FALSE) {
    impanic("SignupApiHandler Constructor takes (struct FordoDB *)");
  }
...
__Handle__(register void *const _self,
  ...
  if (strcmp(HttpRequest_GetPath(request), "/api/signup") != 0) {
  ...
  if (username != NULL && password != NULL) {
    register struct ImResInt const result = FordoDB_AddUser(self->db, username, password);
    if (ImResInt_IsErr(result) != IM_FALSE) {
      register struct ImError *const err = ImResInt_UnwrapErr(result);
      if (imisof(err, ExecuteError)) {
        ImIList_Append(error_list, imnew(ImStr, 1u, PARAM_PTR, "Username is already taken"));
      } else {
...
PRIVATE void __InterfaceImplementation__(register void *const interface) {
  if (imisof(interface, HttpHandler) != IM_FALSE) {
    ...
    impanic("SignupApiHandler does not implement %s\n", imtype(interface));
  }
}

CLASS(SignupApiHandler) {
  _SignupApiHandler.size = sizeof(struct SignupApiHandler);
  _SignupApiHandler.ctor = __Constructor__;
  _SignupApiHandler.dtor = __Destructor__;
  _SignupApiHandler.implof = __InterfaceImplementation__;
}
